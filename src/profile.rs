...
mod tests {
    ...
    fn constructor_should_set_fields_correctly() {
        let profile = Profile::new("default", "prboom", Skill::UltraViolence, true, false).unwrap();
    ...
    fn constructor_should_return_error_for_name_not_set() {
        let result = Profile::new("", "prboom", Skill::UltraViolence, true, false);
    ...
    fn constructor_should_return_error_for_source_port_not_set() {
        let result = Profile::new("default", "", Skill::UltraViolence, true, false);
        assert!(result.is_err());
        assert_eq!(
            result.unwrap_err().to_string(),
            "The source port for the profile must be set"
